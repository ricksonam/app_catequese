...
import {
  ...
  fetchTurmaMembros, removeTurmaMembro, approveTurmaMembro,
...
export function useApproveTurmaMembro() {
  const qc = useQueryClient();
  return useMutation({ 
    mutationFn: ({ turmaId, userId }: { turmaId: string, userId: string }) => approveTurmaMembro(turmaId, userId), 
    onSuccess: (_, variables) => { qc.invalidateQueries({ queryKey: ["turma_membros", variables.turmaId] }); } 
  });
}
...
export function useAllRespostasCount(formIds: string[]) {
  return useQuery({
