...
export async function fetchTurmas(userId?: string): Promise<Turma[]> {
  ...
  const memberStatusMap = new Map((memberData || []).map((m: any) => [m.turma_id, m.status]));
  const sharedIds = Array.from(memberStatusMap.keys());
...
export async function fetchCatequizandos(turmaId?: string): Promise<Catequizando[]> {
  ...
  return (data || []).map((c: any) => ({
    ...
    responsaveis: c.responsaveis || undefined,
...
export async function fetchParoquias(): Promise<(Paroquia & { isShared?: boolean })[]> {
  ...
  // Own records
  const { data: ownData, error: ownError } = await (supabase.from as any)("paroquias")
    .select("*").eq("user_id", user.id).order("nome");
  ...
  // Shared via RPC (SECURITY DEFINER bypasses RLS)
  const { data: sharedData } = await supabase.rpc('get_shared_paroquias' as any);
  const ownIds = new Set(own.map(p => p.id));
  const shared = ((sharedData as any[]) || [])
...
// ========== CATEQUISTAS ==========
export async function fetchCatequistas(): Promise<(CatequistaCadastro & { isShared?: boolean })[]> {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return [];

  const mapC = (c: any, shared: boolean) => ({
  ...
  const { data: ownData, error: ownError } = await (supabase.from as any)("catequistas")
    .select("*").eq("user_id", user.id).order("nome");
  if (ownError) throw ownError;
  const own = (ownData || []).map((c: any) => mapC(c, false));

  // Shared via RPC (SECURITY DEFINER bypasses RLS)
  const { data: sharedData } = await supabase.rpc('get_shared_catequistas' as any);
  const ownIds = new Set(own.map(c => c.id));
  const shared = ((sharedData as any[]) || [])
    .filter((c: any) => !ownIds.has(c.id))
    .map((c: any) => mapC(c, true));
...
export async function approveTurmaMembro(turmaId: string, userId: string) {
  const { error } = await supabase.rpc('approve_turma_membro', {
    p_turma_id: turmaId,
    p_user_id: userId,
  });
  if (error) throw error;
...
export async function fetchPublicTurmaByCode(code: string) {
  const { data, error } = await supabase.rpc('get_public_turma', { p_code: code.trim().toUpperCase() });
  if (error) throw error;
  return data;
}
