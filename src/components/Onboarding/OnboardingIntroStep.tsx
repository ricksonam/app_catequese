    ...
    <Dialog open={open} onOpenChange={() => {}}>
      <DialogContent className="max-w-md w-[95vw] rounded-[32px] p-0 overflow-hidden border-none shadow-2xl bg-white dark:bg-zinc-950 max-h-[92vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-7 pb-7">
          ...
          {/* Header */}
          <div className="flex flex-col items-center text-center pt-8 pb-5">
            <div className="relative mb-4">
              <div className="w-20 h-20 rounded-[28px] overflow-hidden bg-white shadow-xl border border-black/5 p-2.5">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Configuração Inicial</p>
              <Sparkles className="h-3.5 w-3.5 text-amber-500 animate-pulse" />
            </div>
          ...
          <div className="space-y-3 mb-6">
            {etapas.map((e, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`flex items-center gap-4 p-4 rounded-2xl border-2 ${e.bg} ${e.border} transition-all`}
                ...
                >
                  ...
                  <div className="shrink-0 flex flex-col items-center gap-1">
                    ...
                    <span className="text-[9px] font-black text-muted-foreground/60 tracking-widest">{e.step}</span>
                  </div>

                  {/* Texto */}
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-black ${e.iconColor} leading-tight`}>{e.title}</p>
                    <p className="text-[11px] text-muted-foreground leading-snug mt-0.5">{e.desc}</p>
                  </div>
          ...
          <div className="flex items-start gap-3 bg-amber-50 dark:bg-amber-900/10 border border-amber-200/60 dark:border-amber-800/30 rounded-2xl p-3.5 mb-6 text-left">
          ...
          <button
            onClick={onStart}
            className="w-full h-14 rounded-2xl bg-gradient-to-r from-violet-600 via-primary to-emerald-600 text-white font-black text-sm shadow-xl shadow-primary/25 active:scale-[0.97] transition-all hover:brightness-110 flex items-center justify-center gap-2 group"
          >
            Vamos Começar!
