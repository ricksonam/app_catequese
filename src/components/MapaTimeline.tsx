...
import { ChevronRight, CalendarDays, Heart, BookOpen, Flame, Send } from 'lucide-react';
...
export const itinerarioData: TempoType[] = [
  {
    titulo: "PREPARAÇÃO",
...
export function MapaTimeline() {
  const [expandedTempo, setExpandedTempo] = useState<number | null>(0);

  return (
    <div className="relative mt-4 px-1 sm:px-4">
      {/* The central spine */}
          ...
          return (
            <div key={index} className={`relative pl-12 sm:pl-24 animate-in slide-in-from-bottom-4 duration-500`} style={{ animationDelay: `${index * 150}ms` }}>
              ...
              <button 
                ...
                className={`w-full text-left bg-white dark:bg-zinc-900 rounded-[2rem] p-4 sm:p-6 shadow-sm border border-black/5 dark:border-white/5 transition-all duration-300 hover:shadow-lg relative overflow-hidden group ${isExpanded ? 'ring-2 ring-primary/20' : ''}`}
              >
                ...
                <div className="flex items-start gap-3 sm:gap-4">
                  <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center border-2 shrink-0 ${colorClass}`}>
                    <Icon className="w-5 h-5 sm:w-6 sm:h-6" />
                  </div>
                  <div className="flex-1 min-w-0 pt-1">
                    <h3 className="text-sm sm:text-lg font-black text-foreground leading-tight tracking-tight uppercase group-hover:text-primary transition-colors">{tempo.titulo}</h3>
                    {tempo.duracao && (
                      <p className="text-[10px] sm:text-xs font-bold text-muted-foreground mt-0.5 sm:mt-1 tracking-widest uppercase">{tempo.duracao}</p>
                    )}
                  </div>
                ...
                <div className={`grid transition-all duration-300 ease-in-out ${isExpanded ? 'grid-rows-[1fr] opacity-100 mt-4 sm:mt-6' : 'grid-rows-[0fr] opacity-0 mt-0'}`}>
                    ...
                    <div className="space-y-4 pt-4 border-t border-black/5 dark:border-white/5">
                      {tempo.fases.map((fase, fIndex) => (
                        <div key={fIndex} className="bg-zinc-50 dark:bg-zinc-800/50 rounded-2xl p-4 sm:p-5 border border-black/5">
                          <h4 className="font-black text-[11px] sm:text-sm text-foreground uppercase tracking-wider mb-1.5 sm:mb-2">{fase.titulo}</h4>
                          <p className="text-[11px] sm:text-xs font-medium text-foreground/80 mb-3 sm:mb-4">{fase.objetivo}</p>
                              ...
                              <div>
                                ...
                                <ul className="space-y-1.5 pl-1">
                                  {fase.eixos.map((eixo, i) => (
                                    <li key={i} className="text-[10px] sm:text-xs text-muted-foreground flex gap-2">
