...
export function CustomDatePicker({ value, onChange, label }: CustomDatePickerProps) {
  ...
  return (
    <div className="w-full">
      {label && <label className="text-base font-black text-zinc-900 uppercase tracking-widest mb-1 block ml-1">{label.includes("*") ? <>{label.replace("*", "")}<span className="text-red-500">*</span></> : label}</label>}
      <input 
        type="text"
        value={inputValue}
