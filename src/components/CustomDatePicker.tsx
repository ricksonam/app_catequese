...
export function CustomDatePicker({ value, onChange, label }: CustomDatePickerProps) {
  ...
  return (
    <div className="w-full">
      {label && <label className="text-sm font-black text-zinc-900 mb-1.5 block uppercase tracking-wide">{label.includes("*") ? <>{label.replace("*", "")}<span className="text-red-500">*</span></> : label}</label>}
      <input 
        type="text"
        value={inputValue}
