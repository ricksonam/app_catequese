        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="rounded-2xl max-h-[85vh] overflow-y-auto border-border/30">
            ...
            <div className="space-y-3 mt-2">
              ...
              <div><label className="text-xs font-semibold text-muted-foreground mb-1 block">Tipo</label><select value={form.tipo} onChange={(e) => updateField("tipo", e.target.value)} className="form-input"><option>Paróquia</option><option>Área Pastoral</option><option>Escola</option></select></div>
      ...
      {list.length === 0 ? (
        ...
        <div className="space-y-2">{list.map((item, i) => (
          <div key={item.id} className="float-card flex items-center gap-3 px-4 py-3.5 animate-float-up" style={{ animationDelay: `${i * 50}ms` }}>
            ...
            <div className="flex-1"><p className="text-sm font-semibold text-foreground">{item.nome}</p><p className="text-xs text-muted-foreground">{item.tipo}{item.endereco && ` • ${item.endereco}`}</p></div>
