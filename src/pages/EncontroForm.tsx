...
export default function EncontroForm() {
  ...
  const addStep = () => {
    ...
    if (newStepPosition >= 0 && newStepPosition < roteiro.length) {
      ...
      updated.splice(newStepPosition + 1, 0, newStep);
      setRoteiro(updated);
    } else {
  ...
  return (
    <>
        ...
        <div className="animate-float-up" style={{ animationDelay: '120ms' }}>
                    ...
                    <div className="px-4 pb-4 space-y-3 border-t border-border/30 pt-3">
                      ...
                      <div className="flex items-center justify-center gap-3">
                        <button
                          ...
                          disabled={i === 0}
                          className="flex items-center gap-1 text-xs font-semibold text-primary disabled:text-muted-foreground disabled:opacity-40 px-3 py-1.5 rounded-lg bg-primary/10 hover:bg-primary/20 transition-colors"
                        >
                          <ArrowUpCircle className="h-3.5 w-3.5" /> Mover acima
                        </button>
                        <button
                          ...
                          className="flex items-center gap-1 text-xs font-semibold text-primary disabled:text-muted-foreground disabled:opacity-40 px-3 py-1.5 rounded-lg bg-primary/10 hover:bg-primary/20 transition-colors"
                        >
                          <ArrowDownCircle className="h-3.5 w-3.5" /> Mover abaixo
                        </button>
                      </div>
          ...
          {showAddStep ? (
            <div className="float-card p-4 mt-2 space-y-3 animate-scale-in">
              ...
              />
              <div>
                <label className="text-xs font-semibold text-muted-foreground mb-1 block">Inserir após</label>
                <select
                  value={newStepPosition}
                  onChange={(e) => setNewStepPosition(parseInt(e.target.value))}
                  className="form-input"
                >
                  <option value={-1}>No final da lista</option>
                  {roteiro.map((s, i) => (
                    <option key={s.id} value={i}>Após {i + 1}. {s.label}</option>
                  ))}
                </select>
              </div>
              <div className="flex gap-2">
                ...
                <button onClick={() => { setShowAddStep(false); setNewStepLabel(""); setNewStepPosition(-1); }} className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-muted-foreground bg-muted/50 hover:bg-muted transition-colors">
                  Cancelar
                </button>
              </div>
            </div>
      ...
      {/* Floating total time badge - OUTSIDE the scrollable container */}
      {totalTempo > 0 && (
        <div className="fixed bottom-24 right-5 z-[100] animate-scale-in pointer-events-none">
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-primary text-primary-foreground shadow-xl shadow-primary/30 pointer-events-auto">
            ...
            <span className="text-sm font-bold tabular-nums">{formatTempo(totalTempo)}</span>
          </div>
        </div>
      )}
    </>
