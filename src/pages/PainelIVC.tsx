        ...
        {filtrados.map((enc, i) => {
          ...
          return (
            <div
              key={enc.id}
              className={cn(
                "flex items-center gap-2 px-3 py-2 rounded-xl text-[11px] font-bold border-2 transition-all w-full",
                isRealizado
            ...
            >
              {isRealizado ? (
                ...
                <span className="w-5 h-5 rounded-full border-2 border-current flex items-center justify-center text-[9px] font-black shrink-0">
                  {i + 1}
                </span>
              )}
              <span className="font-black shrink-0">{dateFmt}</span>
              {enc.tema && <span className="opacity-80 flex-1 min-w-0 break-words whitespace-normal leading-tight">— {enc.tema}</span>}
            </div>
          );
        })}
      </div>
                  ...
                  <div className={cn(
                    ...
                    <div className="px-5 pb-8 pt-1 space-y-3 max-h-[80vh] overflow-y-auto overscroll-contain">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={cn("text-[10px] font-black uppercase tracking-widest leading-none mb-1", balloonTitleCor)}>Sobre esta etapa</p>
                          <p className="text-base font-black text-foreground leading-tight">{descInfo.titulo}</p>
                        </div>
                        <button
                          onClick={(e) => { e.stopPropagation(); setExpandida(null); }}
                          className="shrink-0 w-8 h-8 rounded-full bg-muted/60 flex items-center justify-center text-muted-foreground hover:bg-muted transition-colors text-sm font-black"
                        >
                          ✕
                        </button>
                      </div>
                      <p className="text-[13px] leading-relaxed text-foreground/80">{descInfo.descricao}</p>

                      {/* Encontros do catecumenato */}
                      {descInfo.showEncontros && encontros && encontros.length > 0 && (
                        <div className="pt-3 border-t-2 border-current/10">
                          <EncontrosChipList
                            encontros={encontros}
