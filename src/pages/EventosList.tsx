    ...
    <div className="space-y-5 pb-10">
      <div className="space-y-4 animate-fade-in flex flex-col pt-4">
        ...
        <div className="flex items-center justify-center min-h-[44px] relative">
          <button onClick={() => navigate(`/turmas/${id}`)} className="w-10 h-10 flex items-center justify-center rounded-xl bg-orange-500 hover:bg-orange-600 border border-orange-600/30 shadow-md shadow-orange-500/20 active:scale-90 transition-all absolute left-0">
          ...
          <div className="flex flex-col items-center gap-1 text-center">
            ...
            <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest leading-none">{list.length} eventos</p>
          </div>
        </div>
          ...
          <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setEditingId(null); setForm({ ...emptyForm }); } }}>
            ...
            <DialogContent className="rounded-2xl max-h-[85vh] overflow-y-auto border-border/30">
              ...
              <div className="space-y-3 mt-2">
                <div>
                  <label className="text-xs font-semibold text-zinc-900 mb-1 block">Tipo de Evento <span className="text-red-500">*</span></label>
                ...
                <FieldInput label="Nome do Evento *" value={form.nome} onChange={(v) => updateField("nome", v)} />
                <div>
                  <label className="text-xs font-semibold text-zinc-900 mb-1 block">Descrição</label>
                  <textarea 
      ...
      {list.length === 0 ? (
        <div className="empty-state animate-float-up"><div className="icon-box bg-liturgical/10 text-liturgical mx-auto mb-3"><ListChecks className="h-6 w-6" /></div><p className="text-sm font-medium text-muted-foreground">Nenhum evento cadastrado</p></div>
      ) : (
