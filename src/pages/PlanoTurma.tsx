...
import { getTurmas, getEncontros, getAtividades } from "@/lib/store";
...
const statusColors: Record<string, string> = {
  pendente: 'bg-muted-foreground', realizado: 'bg-success', adiado: 'bg-warning', transferido: 'bg-caution', cancelado: 'bg-destructive',
};
...
export default function PlanoTurma() {
  ...
  const items: TimelineItem[] = [
    ...
    if (!a.data && !b.data) return 0; if (!a.data) return 1; if (!b.data) return -1;
    ...
    <div className="space-y-5">
      ...
      {items.length === 0 ? (
        <div className="empty-state animate-float-up">
          ...
          <p className="text-sm font-medium text-muted-foreground">Nenhum encontro ou atividade cadastrada</p>
              ...
              return (
                <div key={`${item.tipo}-${item.id}`} className="relative flex items-start gap-3 py-1 animate-float-up" style={{ animationDelay: `${i * 50}ms` }}>
                  ...
                  <button onClick={() => handleOpen(item)} className="flex-1 float-card flex items-center gap-3 p-3.5 text-left">
                    <div className={`icon-box w-9 h-9 rounded-lg ${item.tipo === 'encontro' ? 'bg-primary/10' : 'bg-accent/15'}`}>
