...
import { Plus, Search, ListTodo, PieChart, FileText, ExternalLink, Copy, Share2, Trash2, Pencil, Info, ChevronRight, QrCode, MessageSquare } from "lucide-react";
...
export default function ComunicacaoHub() {
  ...
  const [filter, setFilter] = useState<'todos' | 'pesquisa' | 'questionario' | 'avaliacao' | 'evento'>('todos');
  ...
  const getTypeIcon = (tipo: string) => {
    switch (tipo) {
      ...
      case 'evento': return <Plus className="h-4 w-4" />;
  ...
  const getTypeColor = (tipo: string) => {
    switch (tipo) {
      ...
      case 'evento': return "text-orange-500 bg-orange-500/10 border-orange-200/50";
    ...
    <div className="flex flex-col gap-6 animate-fade-in relative min-h-[80vh]">
            ...
            <div className="p-6 flex flex-col items-center">
              <div className="flex items-center justify-between w-full mb-5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-black text-foreground">QR Code</span>
                </div>
                <button
                  onClick={() => setShowQrCode(null)}
                  className="w-8 h-8 rounded-xl bg-muted flex items-center justify-center hover:bg-muted/80 transition-all active:scale-95"
              ...
              <div className="bg-white p-4 rounded-3xl shadow-inner border border-black/5 mb-4">
                <QRCodeSVG value={getLinkPublico(showQrCode)} size={180} />
              </div>
              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground text-center mb-5">
                Aponte a câmera para participar
              </p>
              <button
                onClick={() => { handleCopyLink(showQrCode); setShowQrCode(null); }}
                className="w-full py-3 bg-purple-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-purple-600/20 active:scale-95 transition-all"
              >
                Copiar Link
              </button>
            </div>
      ...
      <div className="relative flex flex-col items-center gap-2">
        <button
          ...
          className="absolute right-0 top-0 w-8 h-8 rounded-full bg-purple-500/15 text-purple-500 flex items-center justify-center hover:bg-purple-500/30 hover:scale-110 transition-all active:scale-95 shadow-sm border border-purple-500/20"
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {(['todos', 'pesquisa', 'questionario', 'avaliacao', 'evento'] as const).map(f => {
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredForms.map(form => (
            <div 
              key={form.id} 
              className="float-card p-5 cursor-pointer flex flex-col justify-between group h-full relative overflow-hidden"
              ...
              <div>
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="flex gap-4">
                    <button 
                      onClick={(e) => { e.stopPropagation(); setShowQrCode(form.id); }}
                      className="p-2 rounded-xl text-muted-foreground hover:bg-purple-500/10 hover:text-purple-600 transition-all active:scale-95 shadow-sm border border-transparent hover:border-purple-200/50"
                      title="QR Code"
                    >
                ...
                {form.tipo === 'evento' && form.configuracoes?.vagasTotais && (
                  <div className="mb-4">
                    <div className="flex justify-between items-end mb-1">
                      <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Vagas Preenchidas</span>
                      <span className="text-xs font-black text-orange-600">
                        {form.configuracoes.vagasTotais - (form.configuracoes.vagasDisponiveis || 0)} / {form.configuracoes.vagasTotais}
                      </span>
                    </div>
                    <div className="h-1.5 w-full bg-slate-100 dark:bg-zinc-800 rounded-full overflow-hidden border border-black/5">
                      <div 
                        className="h-full bg-orange-500 transition-all duration-500" 
                        style={{ width: `${Math.min(100, ((form.configuracoes.vagasTotais - (form.configuracoes.vagasDisponiveis || 0)) / form.configuracoes.vagasTotais) * 100)}%` }}
                      />
                    </div>
                  </div>
                )}
                ...
                <div className="flex justify-center my-4">
                  <div 
                    onClick={(e) => { e.stopPropagation(); handleCopyLink(form.codigo_acesso); }}
