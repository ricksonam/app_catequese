        ...
        <div className="flex items-center gap-2">
          ...
          <DialogContent className="rounded-2xl max-h-[85vh] overflow-y-auto border-border/30">
            ...
            <div className="space-y-3 mt-2">
              <div><label className="text-xs font-semibold text-muted-foreground mb-1 block">Tipo de Atividade *</label><select value={form.tipo} onChange={(e) => updateField("tipo", e.target.value)} className="form-input font-bold text-primary">{ATIVIDADE_TIPOS.map(t => <option key={t}>{t}</option>)}</select></div>
