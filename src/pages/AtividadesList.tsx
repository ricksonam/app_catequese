  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between animate-fade-in">
        <div className="flex items-center gap-3"><button onClick={() => navigate(`/turmas/${id}`)} className="back-btn"><ArrowLeft className="h-5 w-5 text-foreground" /></button><div><h1 className="text-xl font-bold text-foreground">Atividades</h1><p className="text-xs text-muted-foreground">{turma?.nome} • {list.length} atividades</p></div></div>
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setEditingId(null); setForm({ ...emptyForm }); } }}>
          ...
          <DialogContent className="rounded-2xl max-h-[85vh] overflow-y-auto border-border/30">
            ...
            <div className="space-y-3 mt-2">
              ...
              <div><label className="text-xs font-semibold text-muted-foreground mb-1 block">Tipo</label><select value={form.tipo} onChange={(e) => updateField("tipo", e.target.value)} className="form-input">{ATIVIDADE_TIPOS.map(t => <option key={t}>{t}</option>)}</select></div>
        ...
        <DialogContent className="rounded-2xl border-border/30 p-0 overflow-hidden max-h-[90vh] overflow-y-auto">
          {viewItem && <>
            <div className={`px-5 pt-5 pb-4 ${tipoColors[viewItem.tipo]?.replace(/text-\S+/, '') || 'bg-muted'}`}>
              <span className={`inline-block pill-btn text-[10px] mb-2 ${tipoColors[viewItem.tipo] || 'bg-muted text-muted-foreground'}`}>{viewItem.tipo} • {viewItem.modalidade === 'externa' ? 'Externa' : 'Interna'}</span>
            ...
            <div className="px-5 pb-5 space-y-4">
              ...
              {viewItem.descricao && <div><p className="text-xs font-semibold text-muted-foreground mb-1">Descrição</p><p className="text-sm text-foreground">{viewItem.descricao}</p></div>}
              {viewItem.observacao && <div><p className="text-xs font-semibold text-muted-foreground mb-1">Observação</p><p className="text-sm text-foreground">{viewItem.observacao}</p></div>}
              <div className="flex gap-2 pt-2">
                <button onClick={() => { setPresencaItem(viewItem); setPresencaOpen(true); }} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-success/10 text-success text-xs font-semibold"><Users className="h-3.5 w-3.5" /> Presença ({(viewItem.presencas||[]).length})</button>
                {viewItem.modalidade === 'externa' && <button onClick={() => printAutorizacao(viewItem)} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-primary/10 text-primary text-xs font-semibold"><Printer className="h-3.5 w-3.5" /> Autorização</button>}
              </div>
            </div>
          </>}
        </DialogContent>
      ...
      <Dialog open={presencaOpen} onOpenChange={setPresencaOpen}>
        <DialogContent className="rounded-2xl border-border/30"><DialogHeader><DialogTitle>Presença - {presencaItem?.nome}</DialogTitle></DialogHeader>
