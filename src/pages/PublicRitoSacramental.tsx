...
import { CalendarDays, Heart, BookOpen, Music, Star, Calendar, CheckCircle2 } from "lucide-react";
import Spinner from "@/components/ui/spinner";

type SacramentoType = "batismo" | "eucaristia" | "crisma";

const ETAPAS_RITO = [
  { key: "reuniao_pais", label: "Reunião com os pais", icon: Heart, color: "text-rose-600", bg: "bg-rose-50 border-rose-200", iconBg: "bg-rose-500" },
  { key: "confissao", label: "Celebração Penitencial (Confissão)", icon: BookOpen, color: "text-violet-600", bg: "bg-violet-50 border-violet-200", iconBg: "bg-violet-500" },
  { key: "retiro", label: "Retiro Espiritual", icon: Star, color: "text-blue-600", bg: "bg-blue-50 border-blue-200", iconBg: "bg-blue-500" },
  { key: "ensaio", label: "Ensaio do Rito", icon: Music, color: "text-emerald-600", bg: "bg-emerald-50 border-emerald-200", iconBg: "bg-emerald-500" },
  { key: "confraternizacao", label: "Confraternização", icon: Star, color: "text-amber-600", bg: "bg-amber-50 border-amber-200", iconBg: "bg-amber-500" },
] as const;

const SACRAMENTO_LABELS: Record<string, { label: string; color: string; gradient: string }> = {
  batismo: { label: "Batismo", color: "text-sky-700", gradient: "from-sky-500 to-blue-600" },
  eucaristia: { label: "Primeira Eucaristia", color: "text-amber-700", gradient: "from-amber-500 to-orange-600" },
  crisma: { label: "Crisma", color: "text-violet-700", gradient: "from-violet-500 to-purple-600" },
};
...
export default function PublicRitoSacramental() {
  ...
  // Buscar etapas do rito para o sacramento selecionado
  const ritoConfig =
  ...
  return (
    <div className="min-h-screen bg-[#FAFBFF] dark:bg-zinc-950 pb-24">
      {/* Header */}
      <div className={`bg-gradient-to-br ${sacInfo.gradient} text-white px-6 py-8 shadow-lg`}>
        <div className="max-w-lg mx-auto text-center space-y-3">
          <div className="w-14 h-14 bg-white/20 rounded-2xl flex items-center justify-center mx-auto mb-1 shadow-lg shadow-black/10">
      ...
      <div className="max-w-lg mx-auto px-5 mt-6 space-y-5">
        ...
        {proximaEtapa && (
          <div className="relative overflow-hidden bg-gradient-to-br from-primary/5 to-primary/10 rounded-2xl border-2 border-primary/20 p-5">
        ...
        {/* Lista de Etapas */}
        <div>
          <p className="text-xs font-black uppercase tracking-widest text-muted-foreground mb-3 px-1">Todas as Etapas</p>
              ...
              return (
                ...
                >
                  ...
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${
                    done ? "bg-emerald-500" : dateVal ? etapa.iconBg : "bg-muted"
                  }`}>
                    {done ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {dateVal ? (
                      <p className={`text-xs mt-0.5 font-semibold capitalize ${done ? "text-emerald-600" : etapa.color}`}>
                        {today ? "🌟 Hoje! " : ""}{formatData(dateVal)}
                      </p>
                    ) : (
                      <p className="text-xs mt-0.5 text-muted-foreground italic">Data não definida</p>
                    )}
                  </div>

                  {/* Badge status */}
                  {today && (
                    <span className="shrink-0 px-2 py-1 rounded-lg bg-amber-500 text-white text-[9px] font-black uppercase tracking-wide">
                      Hoje
                    </span>
                  )}
                  {done && !today && (
                    <span className="shrink-0 px-2 py-1 rounded-lg bg-emerald-100 text-emerald-700 text-[9px] font-black uppercase tracking-wide">
                      Concluído
                    </span>
                  )}
                </div>
