...
export default function ComunidadesCadastro() {
  ...
  const handleAdd = () => {
    ...
    saveComunidade({ id: crypto.randomUUID(), ...form });
    setList(getComunidades()); setForm({ ...emptyForm }); setOpen(false);
    toast.success("Cadastro realizado!");
  };

  const handleDelete = (id: string) => { deleteComunidade(id); setList(getComunidades()); setViewItem(null); toast.success("Removido!"); };
    ...
    <div className="space-y-5">
            ...
            <div className="space-y-3 mt-2">
              ...
              <div>
                ...
                <select value={form.tipo} onChange={(e) => updateField("tipo", e.target.value)} className="form-input">
                  <option>Comunidade</option><option>Núcleo</option><option>Grupo</option>
                </select>
              </div>
              {paroquias.length > 0 && (
                <div>
                  <label className="text-xs font-semibold text-muted-foreground mb-1 block">Paróquia vinculada</label>
                  <select value={form.paroquiaId} onChange={(e) => updateField("paroquiaId", e.target.value)} className="form-input">
      ...
      {list.length === 0 ? (
        <div className="empty-state animate-float-up">
          <div className="icon-box bg-accent/15 text-accent-foreground mx-auto mb-3"><Users className="h-6 w-6" /></div>
