...
const STEP_COLORS: Record<string, string> = {
  oracao_inicial: "from-[hsl(270,45%,50%)] to-[hsl(290,50%,55%)]",
  acolhida: "from-[hsl(38,92%,50%)] to-[hsl(25,95%,53%)]",
  desenvolvimento: "from-[hsl(225,70%,45%)] to-[hsl(220,80%,55%)]",
  atividade: "from-[hsl(152,60%,42%)] to-[hsl(170,55%,45%)]",
  oracao_final: "from-[hsl(270,45%,50%)] to-[hsl(300,50%,55%)]",
  envio: "from-[hsl(200,70%,50%)] to-[hsl(220,80%,60%)]",
};
...
export default function EncontroPresentation() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/2 right-5 w-20 h-20 rounded-full bg-primary/3 blur-2xl animate-pulse" style={{ animationDelay: '2s' }} />
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="px-5 pt-5 pb-3">
          <div className="flex items-start justify-between">
            ...
            <button
              onClick={() => navigate(-1)}
              className="w-8 h-8 rounded-full bg-muted/60 hover:bg-muted flex items-center justify-center transition-all active:scale-90"
            >
        ...
        {/* Step dots */}
        <div className="flex justify-center gap-1 px-5 mb-2">
          {steps.map((s, i) => (
            <button
              key={i}
              onClick={() => {
                setSlideDirection(i > currentStep ? "right" : "left");
                setAnimKey((k) => k + 1);
                setCurrentStep(i);
              }}
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-5 py-4 overflow-y-auto">
          <div
            ...
            style={{
              animation: `${slideDirection === "right" ? "slideInRight" : "slideInLeft"} 0.4s cubic-bezier(0.16, 1, 0.3, 1)`,
            }}
          >
            {/* Step icon & title */}
            <div className="flex flex-col items-center mb-5">
              <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${gradientClass} flex items-center justify-center mb-3 shadow-lg`}>
                <span className="text-2xl">{stepIcon}</span>
              </div>
              <h2 className="text-xl font-bold text-foreground text-center">{step.label}</h2>
              {step.oracaoTipo && (
                <span className="mt-2 inline-block bg-primary/10 text-primary text-xs font-semibold px-3 py-1 rounded-full">
                  {step.oracaoTipo}
                </span>
              )}
            </div>
            ...
            {step.conteudo ? (
              <div className="bg-card/80 backdrop-blur-xl rounded-2xl p-5 shadow-lg shadow-foreground/[0.04] border border-border/30">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs font-semibold text-primary uppercase tracking-wider">Conteúdo</span>
                </div>
                <p className="text-sm text-foreground/90 leading-relaxed whitespace-pre-wrap">{step.conteudo}</p>
              </div>
            ) : (
              <div className="bg-card/40 backdrop-blur-xl rounded-2xl p-8 border border-dashed border-border/50 text-center">
        ...
        <div className="px-5 pb-6 pt-3">
          <div className="flex items-end justify-between">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-sm font-semibold text-foreground disabled:text-muted-foreground/40 px-4 py-3 rounded-2xl bg-card/80 backdrop-blur-sm border border-border/30 shadow-sm hover:shadow-md active:scale-95 transition-all disabled:shadow-none"
            >
            ...
            {step.tempo > 0 ? (
              <button
                ...
                onTouchEnd={(e) => { e.stopPropagation(); e.preventDefault(); handleTimerClick(e); }}
                className="relative -mt-4 group"
              >
                {/* Outer ring with progress */}
                <svg className="w-20 h-20 -rotate-90" viewBox="0 0 80 80">
                  <circle cx="40" cy="40" r="36" fill="none" strokeWidth="3" className="stroke-border/40" />
                  <circle
                    cx="40" cy="40" r="36"
                    fill="none"
                    strokeWidth="3"
                    strokeLinecap="round"
                    className={timerRunning ? "stroke-primary" : timeLeft === 0 ? "stroke-destructive" : "stroke-muted-foreground/30"}
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className={`text-sm font-bold tabular-nums ${
                    timerRunning ? "text-primary" : timeLeft === 0 ? "text-destructive" : "text-foreground"
                  }`}>
                    {formatTime(timeLeft)}
                  </span>
            ...
            <button
              ...
              className={`flex items-center gap-1.5 text-sm font-semibold px-4 py-3 rounded-2xl shadow-sm hover:shadow-md active:scale-95 transition-all disabled:shadow-none ${
                currentStep < steps.length - 1
                  ? `bg-gradient-to-r ${gradientClass} text-white shadow-md`
                  : "bg-card/80 backdrop-blur-sm border border-border/30 text-muted-foreground/40"
              }`}
            >
