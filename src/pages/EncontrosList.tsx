          ...
          {(() => {
            ...
            return Object.entries(groups).map(([month, items], groupIndex) => (
              <div key={month} className="space-y-3">
                <div className="flex items-center justify-center gap-4 py-2 opacity-70">
                  <div className="h-[1px] flex-1 bg-gradient-to-r from-transparent to-muted-foreground/30"></div>
                  <h3 className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em]">{month}</h3>
                  <div className="h-[1px] flex-1 bg-gradient-to-l from-transparent to-muted-foreground/30"></div>
                </div>
                {items.map((enc, i) => {
                    ...
                    <div key={enc.id} className={`float-card overflow-hidden border-l-4 ${status.border} animate-float-up`} style={{ animationDelay: `${(i + 1) * 60}ms` }}>
                      <div className="px-4 py-3.5">
                        ...
                        <div className="flex items-center gap-2 mt-3">
                          <button onClick={() => navigate(`/turmas/${id}/encontros/${enc.id}`)} className="flex items-center gap-1 text-xs font-semibold text-primary bg-primary/10 px-3 py-1.5 rounded-xl active:scale-95 transition-transform">
                            <Eye className="h-3.5 w-3.5" /> Visualizar
                          </button>
                          <button onClick={() => navigate(`/turmas/${id}/encontros/${enc.id}/apresentacao`)} className="flex items-center gap-1 text-xs font-semibold text-liturgical bg-liturgical/10 px-3 py-1.5 rounded-xl active:scale-95 transition-transform">
