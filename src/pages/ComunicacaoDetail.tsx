          ...
          <div>
            <h2 className="text-sm font-black text-foreground uppercase tracking-widest">
              {form.tipo === 'pesquisa' ? 'Resultado da Pesquisa' : 
               ...
               form.tipo === 'evento' ? 'Controle de Inscrições' :
               'Resultado da Avaliação'}
            </h2>
            <p className="text-xs text-muted-foreground mt-0.5">
              {form.tipo === 'evento' && form.configuracoes?.vagasTotais 
                ? `Vagas: ${form.configuracoes.vagasTotais - (form.configuracoes.vagasDisponiveis || 0)} preenchidas de ${form.configuracoes.vagasTotais}`
                : 'Visualize ou exporte as respostas abaixo'}
            </p>
          </div>
        </div>
