          ...
          {viewItem && !editMode && (
            <>
              ...
              <div className="space-y-4 text-sm mt-4">
                ...
                {viewItem.sacramentos && <div><p className="text-xs font-semibold text-muted-foreground mb-1.5">Sacramentos</p>
                  {(["batismo", "eucaristia", "crisma"] as const).map(sac => { const s = viewItem.sacramentos![sac]; return (
                    <div key={sac} className="flex items-center gap-2 py-1.5"><span className={`w-2.5 h-2.5 rounded-full ${s.recebido ? 'bg-success' : 'bg-muted-foreground/30'}`} /><span className="capitalize font-semibold text-foreground">{sac}</span>{s.recebido && s.paroquia && <span className="text-muted-foreground text-xs">• {s.paroquia}</span>}{s.recebido && s.data && <span className="text-muted-foreground text-xs">• {new Date(s.data + 'T00:00').toLocaleDateString("pt-BR")}</span>}</div>
                  ); })}</div>}
              </div>
            </>
          )}
