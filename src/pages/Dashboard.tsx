...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    const fetchNovosMateriaisCount = async () => {
      try {
        const ultimoVisto = localStorage.getItem('ivc_materiais_ultimo_visto');
        let query = supabase
          .from('material_apoio')
          .select('id, publicado_em', { count: 'exact', head: false })
          .eq('ativo', true);
        if (ultimoVisto) {
          query = query.gt('publicado_em', ultimoVisto);
        } else {
          // Show materials from last 7 days if never seen
          const sete = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
          query = query.gt('publicado_em', sete);
        }
        const { count } = await query;
        if (count && count > 0) {
          setNovosMateriaisCount(count);
          setShowMaterialBanner(true);
        }
      } catch (err) {
        console.error('Erro ao checar novos materiais:', err);
      }
    };
    ...
    <div className="space-y-2.5">
      ...
      <div className="relative pt-1 pb-3 mb-0 animate-fade-in">
        ...
        {aniversariantesMes.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-2 px-4 min-h-[60px]">
      ...
      {turmas.length > 0 && (
        <div className="mt-2 animate-fade-in">
                ...
                {turmaModules.map((item, index) => (
                  <div key={index} className="w-full flex flex-col items-center group animate-fade-in" style={{ animationDelay: `${index * 50}ms` }}>
                    ...
                    <div className="min-h-[2.5rem] flex items-start justify-center mt-1 w-full px-0.5">
                      <span className="text-xs sm:text-[13px] font-black text-center uppercase tracking-normal text-black dark:text-white group-hover:opacity-80 transition-opacity duration-300 leading-tight line-clamp-2 w-full break-words">
                        {item.title}
                      </span>
                    </div>
                  </div>
          ...
          <div className="mb-3 -mx-4 sm:mx-0">
            {/* Seção de Módulos Gerais (sem fundo) */}
